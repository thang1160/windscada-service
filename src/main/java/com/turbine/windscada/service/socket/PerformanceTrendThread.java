...
public class PerformanceTrendThread extends Thread {
    private static final Logger LOGGER = Logger.getLogger(PerformanceTrendThread.class.getName());

    @Override
    public void run() {
        LOGGER.info("Performance Trend Thread is running");
        SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
        while (true) {
            try {
                Thread.sleep(5000);
                List<ServerWebSocket> closedList = new ArrayList<>();
                List<ServerWebSocket> currentList = WebSocketHandler.getPerformanceTrendSockets();
